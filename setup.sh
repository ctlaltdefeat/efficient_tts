...
apt-get update && apt install --assume-yes -y git wget curl htop ncdu libsndfile1 ffmpeg nano vim iputils-ping cmake aria2 espeak
git config --global user.name "ctlaltdefeat"
git config --global user.email "ctlaltdefeat@tfwno.gf"
curl https://getcroc.schollz.com | bash
pip install Cython flake8 dill joblib black typeguard librosa gdown ipykernel adabelief-pytorch unidecode inflect
conda install gxx_linux-64 matplotlib
...
ln -s /workspace/datasets datasets